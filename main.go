...
func main() {
	botOptions := []bot.Option{
		...
		// Add new handler for all messages
		bot.WithMessageTextHandler("", bot.MatchTypeContains, handlerTwitterLinks),
...
func handlerTwitterLinks(ctx context.Context, tgBot *bot.Bot, update *models.Update) {
	...
	if len(matches) > 0 {
		...
		for i, match := range matches {
			replacedLinks[i] = strings.Replace(match, "x.com", "vxtwitter.com", 1)
		}
